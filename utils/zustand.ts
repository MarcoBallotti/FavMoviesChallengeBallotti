...
export const useLikedStore = create<LikedStore>()(
	persist(
		(set, get) => ({
			likedMovies: [],
			addToLiked: (movieId) => set((state) => ({ likedMovies: [...state.likedMovies, movieId] })),
			removeFromLiked: (movieId) => set((state) => ({ likedMovies: state.likedMovies.filter((id) => id !== movieId) }))
		}), {
		name: 'likedStore',
